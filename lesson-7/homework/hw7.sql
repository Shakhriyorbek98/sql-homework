...
select  count(*) as total_number_rows from customers


----4.Write a query to count the number of unique product categories from the Products table.
	
	select category, 
			count(productid) as unique_product_categories 
		from products 
	group by category
...
-----5. Write a query to find the total sales amount for the product with id 7 in the Sales table.
	...
	select ProductID, 
		sum(saleamount) as amount_product_id7 
		from sales 
		where productid=7
	group by ProductID
...
-----6. Write a query to calculate the average age of employees in the Employees table.
	...
	select avg(age) as average_age_employees from employees

-----7. Write a query to count the number of employees in each department.

	select departmentname,
		count(employeeid) as employees_amount
		from employees 
...
------9. Write a query to calculate the total sales per Customer in the Sales table.
	...
	select customerid,
		sum(saleamount) as sale_per_customer
		from sales
	group by customerid
...
(DeptID is enough, if you don't have DeptName).*/
	...
	select DepartmentName,
		avg(salary) as avg_salary_per_dep,
		count(employeeid) as employees_working_each_dep
		from employees
	group by DepartmentName
...
(DeptID is enough, if you don't have DeptName).*/
	...
	select DepartmentName,
		avg(salary) as avg_salary_expenses
		from Employees
	group by DepartmentName
